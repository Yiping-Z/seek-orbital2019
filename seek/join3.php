<?php
    ...
    $sql = mysqli_query($con, "SELECT * FROM roommie WHERE ida = '$id'"); 
      if (mysqli_num_rows($sql) != 0) {
        ...
        if (strpos($row['participant'], $username) !== false || $row['id'] === $username) {
                    echo '<script type = "text/javascript"> alert("You have already registered")</script>';
                            echo '<script>window.location.href = "roommie.php";</script>';
                }  else {
    $values = implode(" ", $p);  
    if ($check = mysqli_query($con, "UPDATE roommie SET participant = '".$values."' WHERE ida ='".$id."'")) {
